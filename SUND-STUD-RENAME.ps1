...
#- - - - - - ->
#
# Script  : SUND-STUD-RENAME.ps1
# Runs    : Every day 21:00 CET Task Scheduler psscheduled
# Login   : 
# Purpose : Add security-groups from template user
# Author  : 
# Input   : RENAME-SUND-Students-SET-1.csv
# From    : SUND-Students-Search-AD-for-IDM-Changes.ps1
# Output  : $script:logdato-Groups-RENAME-Students.csv
# Mail    : 
...
$script:logdato | out-file ".\$script:logdato-Groups-RENAME-Students.csv" -Append

Start-Transcript -Path ".\$script:logdato-transcript-stud-rename.log"
...
$script:testcreate = Test-Path '.\RENAME-SUND-Students-SET-1.csv'
if ($script:testcreate -eq $false) {
write-host -foregroundcolor red 'RENAME-SUND-Students-SET.csv missing'
Stop-ScriptZipData
} # end if
...
END {start-sleep -s 10}

} # end function

function Get-STUDIERETNcodes {

foreach ($script:group in (Get-ADUser -Identity allemployee -Server "dc3.sund.root.ku.dk" -Properties MemberOf).MemberOf) {   remove-ADGroupMember -Identity $script:group -Members "$script:USERNAME" -Confirm:$false; }
...
"------" >> ".\$script:logdato-Groups-RENAME-Students.csv"
"$script:USERNAME" >> ".\$script:logdato-Groups-RENAME-Students.csv"
...
$MailBody = " `
 `
RENAME students GROUPS $script:logdato `
 `
 ` 
...
'From'='do-not-reply@sund.ku.dk'
          'Subject'="$script:logdato-Groups-RENAME-Students.csv"
          'Body'="$MailBody"
          'Attachments'=".\$script:logdato-Groups-RENAME-Students.csv"
...
Move-Item '.\RENAME-SUND-Students-SET-1.csv' -Destination ".\DATA\$script:logdato-RENAME-SUND-Students-SET-1.csv"
...
Get-Content -Path '.\RENAME-SUND-Students-SET-1.csv' | ForEach-StudentADuser -FunctionName "Get-STUDIERETNcodes"

start-sleep -Seconds 60

Get-Content -Path '.\RENAME-SUND-Students-SET-1.csv' | ForEach-StudentADuser -FunctionName "MemberOfGroups"
